...
def make_ground_truth_gif(save_root, img_tensor, idx):
    ...
    new_list[0].save(
        os.path.join(save_root, f"seq{idx}/ground_truth.gif"),
        format="GIF",
...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        self.make_gif(
            generated_frame[0],
            os.path.join(self.args.save_root, f"seq{idx}/pred_seq.gif"),
        )
        self.make_gif(
            label_frame[0], os.path.join(self.args.save_root, f"seq{idx}/pose.gif")
        )
