    ...
    def __init__(self):
        ...
        with open("train.json", "r") as file:
            ...
            for img_name, img_condition in dict.items():
                ...
                self.img_conditions.append(
                    [self.classes[condition] for condition in img_condition]
                )
        self.transformations = transforms.Compose(
            [
                transforms.Pad(
                    padding=(0, 40, 0, 40), fill=(255, 255, 255), padding_mode="edge"  # type: ignore
                ),
...
def get_test_conditions():
    ...
    for i in range(len(test_conditions_list)):
        for condition in test_conditions_list[i]:
            labels[i, int(classes[condition])] = 1.0

    return labels


def get_new_test_conditions():
    """
    :return: (#test conditions, #classes) tensors
    """
    with open("objects.json", "r") as file:
