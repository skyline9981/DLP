...
def main():
    ...
    diffusion = Diffusion(img_size=args.size, device=device)  # type: ignore
    ...
    for i in range(1, args.iter + 1):
        ...
        sampled_images1 = torch.empty(0, 3, 64, 64).to(device)
        for j in range(int(len(test_conditions) / 4)):  # type: ignore
            sampled_images_tmp = diffusion.sample(
                model, n=4, labels=test_conditions[j * 4 : j * 4 + 4]
            )
            sampled_images1 = torch.cat((sampled_images1, sampled_images_tmp), dim=0)
            print("finish: " + str(j * 4) + " to " + str(j * 4 + 4))

        # sampled_images1 = diffusion.sample(
        #     model, n=len(test_conditions), labels=test_conditions
        # )
        save_images(
            sampled_images1, os.path.join(args.out_dir, f"demo_1_{i}.png"), nrow=8
        )
    ...
    for i in range(1, args.iter + 1):
        torch.manual_seed(i)
        print("Torch seed: " + str(i))
        sampled_images2 = torch.empty(0, 3, 64, 64).to(device)
        for j in range(int(len(new_test_conditions) / 4)):  # type: ignore
            sampled_images_tmp = diffusion.sample(
                model, n=4, labels=new_test_conditions[j * 4 : j * 4 + 4]
            )
            sampled_images2 = torch.cat((sampled_images2, sampled_images_tmp), dim=0)
            print("finish: " + str(j * 4) + " to " + str(j * 4 + 4))
        # sampled_images2 = diffusion.sample(
        #     model, n=len(new_test_conditions), labels=new_test_conditions
        # )
        save_images(
            sampled_images2, os.path.join(args.out_dir, f"demo_2_{i}.png"), nrow=8
        )
