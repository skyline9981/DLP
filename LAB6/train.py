...
parser.add_argument(
    "--checkpoint",
    type=str,
    default="./checkpoint/08-25-00-50-05_DDPM/model/0064ckpt.pt",
)
parser.add_argument("--load", type=bool, default=True)
parser.add_argument("--now_epoch", type=int, default=64)
...
def train():
    ckpt_loc = os.path.join(
        args.save_dir, f'{datetime.today().strftime("%m-%d-%H-%M-%S")}_DDPM'
    )
    ...
    train_loader = DataLoader(
        dataset_train, batch_size=args.bs, shuffle=True, num_workers=args.num_workers
    )
    eval_model = evaluation_model()

    print("Starting Training Loop...")
    train_bar = tqdm(total=(args.ep - args.now_epoch))
    for epoch in range(args.now_epoch + 1, args.ep + 1):
        total_loss = 0
        ...
        if epoch % 1 == 0:
            ...
            ema_sampled_images1 = diffusion.sample(
                ema_model, n=args.bs, labels=test_conditions[: args.bs]
            )
            # save_images(sampled_images1, os.path.join(img_loc, f"sample_1_{epoch}.png"),nrow=8)
            save_images(
                ema_sampled_images1,
                os.path.join(img_loc, f"ema_1_{epoch}.png"),
                nrow=args.bs,
            )
            # new_test
            # sampled_images2 = diffusion.sample(model, n=len(new_test_conditions), labels=new_test_conditions)
            ema_sampled_images2 = diffusion.sample(
                ema_model, n=args.bs, labels=new_test_conditions[: args.bs]
            )
            # save_images(sampled_images2, os.path.join(img_loc, f"sample_2_{epoch}.png"),nrow=8)
            save_images(
                ema_sampled_images2,
                os.path.join(img_loc, f"ema_2_{epoch}.png"),
                nrow=args.bs,
            )
            # save & evaluate
            torch.save(model.state_dict(), os.path.join(mod_loc, f"{epoch:0>4}ckpt.pt"))
            torch.save(
                ema_model.state_dict(), os.path.join(mod_loc, f"{epoch:0>4}ckpt.pt")
            )
            # ema_sampled_images1 = transforms.Resize([64, 64])(ema_sampled_images1)
            score1 = eval_model.eval(
                ema_sampled_images1.float(), test_conditions[: args.bs]
            )
            # ema_sampled_images2 = transforms.Resize([64, 64])(ema_sampled_images2)
            score2 = eval_model.eval(
                ema_sampled_images2.float(), new_test_conditions[: args.bs]
            )
